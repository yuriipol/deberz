...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(signUpOperation.pending, (state) => {
      state.isLoading = true;
      state.error = null;
    });
    builder.addCase(signUpOperation.fulfilled, (state, { payload }) => {
      state.user = { ...payload };
      state.isLogin = true;
      state.isLoading = false;
      state.error = null;
    });
    builder.addCase(signUpOperation.rejected, (state, { payload }) => {
      state.isLogin = false;
      state.isLoading = false;
      state.error = payload;
    });
